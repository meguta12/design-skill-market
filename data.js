...
const CREATORS = [
  {
    ...
    initial: "K",
    avatarColor: "#4f46e5",
    bio: "Design Skill Market の運営者。制作代行サービスも担当しています。「選べない・崩れる」をなくすのがミッション。掲載中のサンプルスキルはすべて運営が制作しています。",
    links: {
      homepage: "",
      x: "",
      instagram: "",
...
## 完成サンプルの構成（商品ページのプレビュー画像と同じものを作る）

1. ヘッダー: 左ロゴ＋右ナビ＋青の「お問い合わせ」ボタン
2. ヒーロー: 左にキャッチコピー「テクノロジーで、ビジネスに信頼と成長を。」風の2行見出し＋説明文＋ボタン2つ／右にオフィス街の写真（青グラデのオーバーレイ）
3. 事業紹介: 線アイコン付きカード4枚（システム開発／クラウド・インフラ／データ分析／UI・UXデザイン等）。各カードに「詳しく見る→」リンク
4. 私たちについて: 左にオフィス写真2枚／右に強みのリスト（チェックアイコン＋見出し＋短文 ×4）
5. 採用情報: 写真＋テキストの横並びセクション
6. CTAバンド: 濃紺背景「まずはお気軽にご相談ください」＋白抜きボタン
7. フッター: 濃紺・4カラム（会社情報／サービス／会社案内／お問い合わせ）
`,
  },
...
1. ヘッダー: 透過ダーク＋ロゴ＋ナビ＋グラデの「無料で始める」
2. ヒーロー: 左に「データを、成長のエンジンに。」風の大見出し＋サブ＋CTA2つ／右にダッシュボードのスクリーンショット風モック（KPIカード＋折れ線）
3. 実績バー: 数値3つ（導入社数・継続率など）を横並び
4. 機能紹介: 紫グラデの丸アイコン＋カード 3×2（ダッシュボード／イベント分析／レポート自動化など）
5. 連携セクション: 「お気に入りのツールとシームレスに連携」＋ Google Analytics / Slack / HubSpot / Stripe などのロゴチップ
6. 料金プラン: 4カード横並び（スターター¥0／プロ¥2,980／ビジネス¥9,800／エンタープライズ要相談）。中央プランをグラデ枠＋「人気」バッジで強調
7. 最終CTA: グラデ全幅バンド「今すぐ、データドリブンな組織へと進化しませんか？」＋ボタン2つ
`,
  },
...
1. ヘッダー: 中央寄りロゴ（円形の家紋風マーク＋屋号）＋細いナビ＋紺の「ご予約・お問い合わせ」
2. ヒーロー: 縁側と庭の写真を全幅で。左上に縦書きで「静けさに、心ほどける。」＋小さな説明文。右に暖簾の写真が重なる
3. コンセプト: 左に坪庭の写真／右に「和の美意識を、今の心地よさへ。」＋短い文章＋「詳しく見る→」
4. お部屋・お料理: 写真カード2枚横並び（くつろぎの設え／旬を味わう会席）。各カードに短文＋リンク
5. ギャラリー: 横長写真4枚を1行で（露天風呂・廊下・灯り・料理など）
6. アクセス: 左に住所・電車/車での行き方のテキスト＋右に地図
7. 予約バンド: 濃紺背景「ご予約・お問い合わせ」＋電話番号 大きく＋Web予約ボタン
8. フッター: 濃紺・最小限
`,
  },
...
1. ヘッダー: 左にイニシャルロゴ（例: Y.）＋右ナビ（作品／プロフィール／お問い合わせ）
2. ヒーロー: 左に名前を特大で＋肩書き「デザインと開発で、伝わる体験をつくる。」風の1文＋ボタン2つ（白黒）＋SNSアイコン行／右に建築写真などモノクロの大きな写真
3. 選りすぐりの作品: 3カラムの作品グリッド（スクリーンショット6点）。各作品の下に小さくタイトル＋種別
4. プロフィール: 左に文章（シンプルなデザインで、確かな実装で価値を届ける。風の見出し＋段落）／右にモノクロのポートレート写真
5. スキル・ツール: 4カラムのテキストリスト（デザイン／フロントエンド／バックエンド／その他）
6. お問い合わせ: メールアドレスを大きく＋SNSリンク小さく＋右に小さな植物写真
7. フッター: © のみ
`,
  },
...
1. ヘッダー: 葉のアイコン＋院名＋診療時間の小さな表記＋緑の「WEB予約」ボタン
2. ヒーロー: 「地域の、やさしいかかりつけ医でありたい。」＋医師と親子の写真。下に診療時間・休診日のカードを重ねる
3. 診療内容: 丸アイコンのカード6枚（一般内科／小児科／予防接種／健康診断／皮膚科／生活習慣病など）＋緑の「お問い合わせ」ボタン
4. 医師のご紹介: 白衣の写真＋名前＋経歴のカード3枚横並び
5. 診療時間と受付について: 診療時間テーブル（○と−）＋右に電話番号を大きく＋注意書き
6. クリニック案内: 院内写真の横並び（受付・待合室・診察室・外観）
7. アクセス・お問い合わせ: 地図＋住所・最寄り駅＋駐車場情報
`,
  },
...
2. ヒーロー: 店内写真を全幅・暗めに。「暮らしに、やさしいひとときを。」＋説明2行＋「メニューを見る」ボタン。**下端は波形（曲線）でクリーム色につなぐ**
3. おすすめメニュー: 写真付きカード4枚（写真上／名前／説明1行／価格）。1枚目に赤の「人気No.1」リボン。下に「メニュー一覧へ」ボタン
4. ギャラリー: 正方形写真5枚を1行で＋「もっと見る」ボタン
5. コンセプト・ストーリー／店舗情報: 左右2分割の濃ブラウンのパネル。左は写真＋文章、右は住所・営業時間・定休日・アクセスのリスト＋外観写真
6. ご予約・お問い合わせ: クリーム背景の中央寄せ。「Webで予約する」ブラウンボタン＋「電話で問い合わせる 03-xxxx-xxxx」白ボタン
7. フッター: 濃ブラウン・ロゴ＋ナビ再掲＋SNSアイコン

※メニューが多い業態（ベーカリー等）では、このカード型の代わりに本文中のドットリーダー型メニュー表を使ってもよい
`,
  },
...
1. 表紙: 左60%は白地に「中期成長を実現するための戦略提案」の小見出し＋「事業変革による持続的な成長の実現」の大タイトル＋リード文＋社名ロゴ・提案日。右40%は都市風景写真を斜めの帯でトリミングし、写真の上に濃紺の「本提案のキーメッセージ」ボックス（白文字で結論1文）を重ねる
2. 目次: 番号付きリスト7項目（左側に縦に並べる）
3. 現状分析と課題: キーメッセージ行＋白カード3枚（業務プロセス／データ活用／組織・人材、各カードに箇条書き3点）＋下部に結論バー
4. 市場環境と機会: 左に積み上げ棒グラフ（CAGRラベル付き）／右に主要トレンドのリスト3点
5. 提案するアプローチ: 中央の円（変革を支える4つの柱）＋四隅にラベルと説明
...
1. 表紙: ピンクの吹き出しバッジ「ゼロからわかる！」＋特大タイトル「学びが深まる勉強法のコツ」＋「〜今日から実践できる5つのステップ〜」＋角丸アイコンカード3枚（目標設定/学習計画/ふり返り）＋下端に緑の講師バー（似顔絵アイコン＋講師名）
2. 今日のポイント: 番号丸付きのパステル円カード3枚（01目標を決める/02計画を立てる/03ふり返って改善）＋各2行の説明
3. 学習の5ステップ: 番号付きの丸アイコンを横一列に並べたタイムライン（点線でつなぐ）
4. 目標設定のコツ: 左に的と矢のイラスト／右にチェックリスト3点＋下に例文バー「例)毎日30分、英単語を50個覚える！」
5. おすすめの学習スケジュール例: 時間×曜日のパステル表（ヘッダー行は緑）＋下にポイントバー
6. ふり返りで成長を実感！: チェックリスト3点＋右にガッツポーズの人物イラスト＋下に黄色の補足バー
7. まとめ: 5ステップのアイコン再掲＋「今日から一歩ずつ、理想の自分を目指そう！」の緑リボン
`,
  },
...
タスク管理アプリを例に、以下の4画面を作る:

1. **ホーム**: 「おはようございます👋」の挨拶＋ベルアイコン → 青の進捗カード（今週のタスク 12/18件・プログレスバー・完了率%）→ クイックアクセス4つ（タスク/プロジェクト/カレンダー/レポート）→ 今日の予定（時刻＋色付き縦線のタイムライン3件）→ 最近のプロジェクト（進捗バー付きカード）
2. **タスク一覧**: 上部に検索・フィルタアイコン → セグメント（すべて/進行中/完了/保留）→ 「今日・5月20日（月）」の日付グループ見出し → タスク行（丸チェックボックス＋タスク名＋優先度チップ 高=赤/中=黄/低=グレー＋プロジェクト名＋時刻）→ 右下に青の＋FAB
3. **タスク詳細**: 戻る矢印＋優先度バッジ → タイトル＋プロジェクト名 → 説明文 → 期限/担当者/タグのメタ行（アイコン付き）→ チェックリスト（済みはチェック＋取り消し線）＋「＋項目を追加」→ 添付ファイル（PDFカード）→ 下部固定の「タスクを完了にする」ボタン
4. **設定**: プロフィール行（アバター＋名前＋メール）→ グループ化リスト: アカウント（プロフィール/アカウント情報/パスワードの変更/二段階認証）→ 通知設定（トグル）→ 表示設定（ダークモード トグル/言語/文字サイズ）→ サポート
- 全画面下部に5タブのタブバー（中央が青の＋ボタン）
`,
  },
  {
...
SaaS管理画面「NEXUS」風の1画面を作る:

1. **左サイドバー**: ロゴ → メニュー10項目（ダッシュボード/分析/ユーザー管理/プロジェクト/サブスクリプション/請求管理/レポート/アラート/設定）→ 下部に「現在のプラン」カード（プラン名＋契約期間＋使用量バー）→ ログアウト
2. **ヘッダー行**: ページタイトル「ダッシュボード」＋説明1行。右に検索・通知・テーマ切替・アバター＋名前
3. **KPIカード4枚**: 総アクティブユーザー/今月の売上/コンバージョン率/データ利用量。各カードに丸アイコン＋大きな数値＋「↑12.5% 前月比」＋右下にミニ棒グラフ
4. **チャート行**: 左2/3に折れ線「アクティブユーザー推移」（ホバーで日付＋値のツールチップ、期間セレクト付き）／右1/3にドーナツ「ユーザーのデバイス内訳」（中央に合計値、凡例は%付き）
5. **最近のアクティビティ**: テーブル（アバター＋名前＋メール/アクション/プロジェクト/日時/ステータスバッジ「成功」）＋「すべてのアクティビティを見る→」
6. **右レール**: フィルター（期間/プロジェクト/プランのセレクト＋適用ボタン）→ リアルタイム状況（●ライブ、オンラインユーザー・セッション・新規登録・システム正常性）→ クイックアクション4件（→付きリスト）
`,
  },
];
